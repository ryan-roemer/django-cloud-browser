...
class Config(object):
  ...
  def connection(self):
    ...
    if not self.__rs_conn_set:
      self.__rs_conn = cloudfiles.get_connection(username=self.account,
  ...
  def get_credentials(cls, cfg, section):
    ...
    if (cfg.has_section(section) and
      ...
      if cfg.has_option(section, cls.opt_servicenet):
        creds['rs_servicenet'] = cfg.getboolean(section, cls.opt_servicenet)
  ...
  def from_settings(cls):
    ...
    if obj is None:
      ...
      obj = cls.from_file(getattr(settings, "CLOUD_BROWSER_CONFIG_FILE", None))
